...
const REMIND_TIME = process.env.REMIND_TIME || "17:00"; // время напоминания (TZ контейнера)
...
setInterval(async () => {
  ...
  for (const u of remindUsers.all()) {
    try {
      ...
      const msg = streak > 0
        ? `Сегодня тренировка? 🥋 Твоя серия — ${streak} 🔥 Не дай ей прерваться!`
        : "Сегодня тренировка? 🥋 Отметь её в приложении и начни новую серию 🔥";
